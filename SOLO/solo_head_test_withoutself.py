...
seg_num_grids = num_grids
cate_out_channels = num_classes - 1
...
mask_loss_cfg = mask_loss_cfg
cate_loss_cfg = cate_loss_cfg
postprocess_cfg = postprocess_cfg
    ...
    def _init_layers():
        ...
        cate_head = nn.ModuleList()
        ins_head = nn.ModuleList()
        cate_out = nn.ModuleList()
        ins_out_list = nn.ModuleList()
        ...
        for i in range(stacked_convs):  
            cate_head.append(
                nn.Sequential(
                    nn.Conv2d(in_channels, seg_feat_channels, kernel_size=3, stride=1, padding=1, bias=False),
                    nn.GroupNorm(num_groups, seg_feat_channels),
                    nn.ReLU(inplace=True)
                ))
        #for last layer, its told to use nclasses -1 =>4 and keep bias true
        cate_out.append(nn.Sequential(
            nn.Conv2d(seg_feat_channels, cate_out_channels, kernel_size=3, stride=1, padding=1, bias=True),
            nn.Sigmoid()
        ))
        cate_out = nn.ModuleList(cate_out)
        ...
        ins_head.append(
            nn.Sequential(
                nn.Conv2d(in_channels + 2, seg_feat_channels, kernel_size=3, stride=1, padding=1, bias=False),
                nn.GroupNorm(32, seg_feat_channels),
                nn.ReLU(inplace=True)
            )
        )
        ...
        for i in range(1, stacked_convs):
            ins_head.append(
                nn.Sequential(
                    nn.Conv2d(seg_feat_channels, seg_feat_channels, kernel_size=3, stride=1, padding=1, bias=False),
                    nn.GroupNorm(32, seg_feat_channels),
                    nn.ReLU(inplace=True)
                )
            )
        ...
        num_grids_2 = [x * x for x in seg_num_grids]
        
        # Add the final output layers to the mask branch
        for out_channels in num_grids_2:
            ins_out_list.append(
                nn.Sequential(
                    nn.Conv2d(seg_feat_channels, out_channels, kernel_size=1, padding=0, bias=True),
                    nn.Sigmoid()
                )
            )
    ...
    def _init_weights():
        ...
        cate_head.apply(xavier_init)
        ins_head.apply(xavier_init)
        cate_out.apply(xavier_init)
        for layer in ins_out_list:
    ...
    def forward(
                fpn_feat_list,
                eval=False):
        new_fpn_list = NewFPN(fpn_feat_list)  # stride[8,8,16,32,32]
